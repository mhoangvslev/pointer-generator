...
class SummarizationModel(object):
  ...
  def _add_placeholders(self):
    ...
    if hps.mode=="decode" and hps.coverage.value:
  ...
  def _add_decoder(self, inputs):
    ...
    prev_coverage = self.prev_coverage if hps.mode=="decode" and hps.coverage.value else None # In decode mode, we run attention_decoder one step at a time and so need to pass in the previous step's coverage vector each time

    outputs, out_state, attn_dists, p_gens, coverage = attention_decoder(inputs, self._dec_in_state, self._enc_states, self._enc_padding_mask, cell, initial_state_attention=(hps.mode=="decode"), pointer_gen=hps.pointer_gen, use_coverage=hps.coverage.value, prev_coverage=prev_coverage)
